...
export function DestinationPage({ country, overview, reasons, fields, requirements, universitySection }: DestinationContent) {
  return <>
    ...
    {universitySection}
    <section className="bg-white py-20"><div className="container-page grid gap-10 lg:grid-cols-2"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-navy">Before you apply</p><h2 className="section-title mt-3">Admission Requirements</h2><p className="mt-4 leading-7 text-slate-600">Requirements vary by university and programme. We help you understand what your chosen institution needs.</p></div><ul className="grid gap-3">{requirements.map((requirement, index) => <li key={requirement} className="flex items-center gap-4 rounded-xl border border-slate-200 p-4"><span className="font-serif text-xl font-bold text-navy">0{index + 1}</span><span>{requirement}</span></li>)}</ul></div></section>
    ...
    <section className="container-page pb-20"><div className="rounded-3xl bg-navy px-7 py-12 text-center text-white sm:px-12"><h2 className="font-serif text-3xl font-bold">Ready to study in {country}?</h2><p className="mt-3 text-slate-200">Speak with Dijon Consultants and start shaping your pathway today.</p><div className="mt-7 flex flex-wrap justify-center gap-4"><CTAButton href="/contact" variant="light">Book Consultation €200</CTAButton><a href="tel:00351925152120" className="inline-flex items-center justify-center rounded-full border border-white px-5 py-3 text-sm font-semibold hover:bg-white hover:text-navy">00351 92 5152 120</a></div></div></section>
  </>;
}
